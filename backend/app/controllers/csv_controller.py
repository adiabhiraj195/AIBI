...
router = APIRouter(prefix="/api/v1", tags=["CSV Management"])
...
async def get_document(
    id: int = Query(..., description="Document ID", gt=0),
    ...
    """
    try:
        document = await CSVService.get_document_by_id(id, include_full_data)
        return document
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Unexpected error fetching document {id}: {e}")
...
@router.patch(
    "/document/{document_id}/description-status",
